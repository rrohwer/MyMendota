  ...
  sidebarLayout(
    sidebarPanel(
      ...
      dateRangeInput(inputId = "date.range",
                     label="Choose a date range",format = "yyyy-mm-dd",min = min(ysi$Timestamp), max=max(ysi$Timestamp),start = min(ysi$Timestamp), end=max(ysi$Timestamp)),
      
      selectInput(inputId="chosen.year",
                  label="Choose a year to highlight on the Secchi Depth plot",
                  choices = list("2014"=2014,"2015"=2015,"2016"=2016,"2017"=2017,"2018"=2018,"2019"=2019),selected=2014),
      
      img(src = "~/Documents/MyMendota/shinyPoseidon/www/trina_lab.png", height = 200, width = 200)
    )
    ,
